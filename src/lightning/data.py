...
from OCTsyntheticdataset.OCTsyntheticDataset import OCTsyntheticDataset
...
class OCTsyntheticDataModule(pl.LightningDataModule):
    """
    For distributed training, each training process is assgined
    only a part of the training scenes to reduce memory overhead.
    """
    ...
    def __init__(self, args, config):
        ...
        self.trainval_data_source = config.DATASET.TRAINVAL_DATA_SOURCE
        self.test_data_source = config.DATASET.TEST_DATA_SOURCE
        self.img_size = config.DATASET.MGDPT_IMG_RESIZE
        self.train_loader_params = {
            'batch_size': args.batch_size,
            'num_workers': args.num_workers,
            'pin_memory': getattr(args, 'pin_memory', True)
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit':
            self.train_dataset = OCTsyntheticDataset(self.trainval_data_source)

            self.val_dataset = OCTsyntheticDataset(self.trainval_data_source)
        else:  # stage == 'test
            self.test_dataset = OCTsyntheticDataset(self.test_data_source)
